...
//            source = "qrc:/image/npc/npc_right.gif"
//        }else if(parent.x === node.x){
//            if(parent.y >= node.y) source = "qrc:/image/npc/npc_up.gif"
//        }
//        else{node1 = node; node2 = parent; source= "qrc:/image/npc/npc_left.gif"}
        ...
        if(parent.x <= node.x){
            ...
            if(parent.y <= node.y){
                if(parent.x === node.x){
                    source = "qrc:/image/npc/npc_up.gif"
                }
                dist[0] = node.x - parent.x + node.width/2 - width/2
                dist[1] = node.y - parent.y - height / 2
            }else{
                if(parent.x === node.x){
                    source = "qrc:/image/npc/npc_down.gif"
                }
                dist[0] = node.x - parent.x + node.width/2 - width/2
                dist[1] = node.y - parent.y - height / 2
            }
        }else{
            source = "qrc:/image/npc/npc_left.gif"
            if(parent.y <= node.y){
                dist[0] = node.x - parent.x + node.width/2 - width/2
                dist[1] = node.y - parent.y - height / 2
            }else{
